...
- **`calcs.py`** — physics. `IPCalculator` solves a cycling ODE (rolling + aero drag + pedaling force, plus cornering: lean angle, CoM/wheel-speed geometry, and an energy-feedback term keyed off `com_height_m`) using `scipy.integrate.solve_ivp` to produce velocity/position/split outputs; a capped `max_step` resolves corner transitions accurately. `TrackShape` generates the track's curvature profile (`kappa(s)`) from `track_length`/`corners` for the cornering model. `CdAFitter` fits CdA from `.fit`-file speed/power data (via `read_fit_file_data`), reusing `IPCalculator`'s cornering statics to account for corner lean effects; recorded speed is wheel/ground speed, not CoM speed, so it's first corrected to an estimated CoM speed (`_recover_com_speed`) before the force-balance inversion.
