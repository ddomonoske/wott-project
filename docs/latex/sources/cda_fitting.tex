...
the recorded speed -- using recorded (center-of-mass) speed rather than the
wheel speed $v_{\mathrm{wheel}}$ of \cref{eq:wheel-speed}, a small approximation
revisited in \cref{sec:assumptions} -- rather than from a separate odometer or
GPS-distance field. Together with the phase offset $s_0$ of
\cref{sec:cda-s0-search}, this distance gives each sample's track position,
$s_i = s_0 + \mathrm{dist}_i$, used for the curvature lookups $\kappa(s_i)$ that
feed the cornering corrections below. Both the unsmoothed finite difference and
the wheel-speed approximation are revisited as limitations in
