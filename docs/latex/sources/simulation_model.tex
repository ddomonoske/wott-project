...
The forward simulation is implemented by \texttt{IPCalculator} and solved
numerically with \texttt{scipy.integrate.solve\_ivp}; the equation of motion
follows.
...
tire-track contact \emph{as if} the rider were riding a surface banked at angle
$\theta$, rather than balancing via tire friction on a flat track: no lateral
friction is needed to supply the centripetal force under this idealization, but
the normal force increases, since the same weight $mg$ must be supported through
...
\texttt{scipy.integrate.solve\_ivp} using its default explicit Runge-Kutta method
...
(\texttt{t\_eval}) of $n = \lceil t_{\max}/dt \rceil$ points spanning
...
silently stepping over a transition zone between two \texttt{t\_eval} samples
...
its chosen steps. Capping \texttt{max\_step} at $dt$ forces the solver to
