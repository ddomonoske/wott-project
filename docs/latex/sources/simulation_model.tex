...
The forward simulation is implemented by \nolinkurl{IPCalculator} and solved
numerically with \nolinkurl{scipy.integrate.solve_ivp}; the equation of motion
follows.
...
$\eta$ (\nolinkurl{mech_losses}) is the fractional mechanical loss through the
drivetrain. Dividing by $m$ gives the straight-line acceleration
\begin{equation}
    a_{\mathrm{straight}} = \frac{F_{rr} + F_{ad} + F_p (1-\eta)}{m}\ .
    \label{eq:a-straight}
\end{equation}
When the track has curvature, an additional cornering correction is added to
\cref{eq:a-straight}; that correction is developed in full in
...
(\nolinkurl{envir.crr}). When the rider is leaned over at angle $\theta$ through a
corner (\cref{sec:cornering-model} derives $\theta$), the model treats the
tire-track contact \emph{as if} the rider were riding a surface banked
precisely at angle $\theta$, rather than balancing via combined normal
force and tire friction on the track surface: no lateral friction is
needed to supply the centripetal force under this idealization, but the
normal force increases, since the same weight $mg$ must be supported through
a smaller effective vertical component:
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:aero-drag}
\end{equation}
where $\rho$ is air density (\nolinkurl{envir.air_density}) and $\CdA$ is the
...
force $F_{\max}$ (\nolinkurl{max_force}, default \SI{200}{\newton}):
\begin{equation}
    F_p(v, t) =
    \begin{cases}
...
\end{equation}
with $F_p = 0$ whenever $v = 0$ and $P(t) \leq 0$.
...
\nolinkurl{scipy.integrate.solve_ivp} using its default explicit Runge-Kutta method
...
imposes on this process is \nolinkurl{max_step}, an upper bound (discussed
...
\end{equation}
detected as a rising zero-crossing (\nolinkurl{direction=1}) so integration halts on
the sample where the rider first reaches $s_{\mathrm{race}}$
(\nolinkurl{race_distance}, default \SI{4000}{\metre}). SciPy locates this crossing
...
(\nolinkurl{t_eval}) of $n = \lceil t_{\max}/dt \rceil$ points spanning
$[0, t_{\max})$ -- a spacing of $t_{\max}/n$, equal to $dt$ when $t_{\max}$ is an
exact multiple of it and slightly under $dt$ otherwise -- independent of the
solver's own internal adaptive step choices. When track geometry is configured, the solver's
\nolinkurl{max_step} is capped at $dt$; when no track geometry is present it is left
unbounded (\nolinkurl{numpy.inf}) so RK45 is free to take its own, larger,
error-controlled steps. The reason for this cap is the curvature profile itself:
...
RK45's own error control, tuned to the comparatively smooth power and drag
dynamics, may grow its step size to several seconds without raising any error,
silently stepping over a transition zone between two \nolinkurl{t_eval} samples
and producing a visibly jagged velocity trace through the corner, since the
solver's dense-output polynomial has no way to know a sharp feature lies between
its chosen steps. Capping \nolinkurl{max_step} at $dt$ forces the solver to
evaluate the right-hand side within every transition zone, at some cost to
integration speed.
...
than one \nolinkurl{t_eval} step of resolution. For CSV export, the velocity and
