...
\begin{equation}
    ...
    \label{eq:ds-dt}
\end{equation}
This is a purely geometric correction; it carries no direct effect on $dv/dt$.
...
(\SI{0.1}{\metre\per\second}), which avoids division-by-near-zero in
\cref{eq:a-energy-general,eq:feedback-coeff}. (Lean angle $\theta$ and the
banked rolling resistance of \cref{sec:rolling-resistance} are not gated this
way; both remain well-defined at $v = 0$, where $\theta \to 0$ smoothly.) This
is a numerical safeguard for the simulation's start condition, not a distinct
physical regime.
