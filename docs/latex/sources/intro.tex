...
\begin{enumerate}
    \item \textbf{Forward race simulation}, implemented by \texttt{IPCalculator}:
    ...
    \item \textbf{Aerodynamic drag fitting}, implemented by \texttt{CdAFitter}:
    given recorded speed and power data from a \texttt{.fit} file, invert the same
...
quasi-static cornering correction that activates when track curvature and rider
center-of-mass height are both known. \Cref{sec:simulation-model} develops this
equation of motion and its numerical solution. \Cref{sec:cornering-model} derives
the cornering correction in detail, since it is the most involved part of the
model. \Cref{sec:track-geometry} describes how the track's curvature profile is
constructed from a small set of geometric parameters. \Cref{sec:cda-fitting}
...
stated otherwise. The one exception is velocity as reported to the user, which is
converted to \si{\kilo\metre\per\hour} purely for display; this conversion is
never used internally and is noted again where it occurs
(\cref{sec:sim-outputs}). \Cref{sec:notation} lists every symbol used in this
document alongside its meaning, unit, and default value.
